...
public class Proj1{
     ...
     */
    public static class Map1 extends Mapper<WritableComparable, Text, Text, DoublePairs> {
        ...
        @Override
            ...
            throws IOException, InterruptedException {
                ...
                // YOUR CODE HERE
				DoublePairs doublePairs = new DoublePairs();
				Text word = new Text();
				ArrayList indices = new ArrayList();
				int index = 0;
				String temp;
				while (matcher.find()){
					if (matcher.group().toLowerCase() == targetGram){
						indices.add(index);
					}
					index++;
				}
				...
				if (!indices.isEmpty()){
					...
					while (matcher.find() && index < indices.get(0)){
						doublePairs.setDouble1(1.0);
						doublePairs.setDouble2(func.f(indices.get(0) - index));
						context.write(word.set(matcher.group().toLowerCase()), doublePairs);
						index++;
					}
					index++;
					matcher.group();
					while (matcher.find()){
						...
						if (temp == targetGram){
							...
							doublePairs.setDouble1(1.0);
							doublePairs.setDouble2(func.f(Math.max(index - indices.get(0), indices.get(1) - index)));
							context.write(word.set(temp), doublePairs);
						}
					...
					while(matcher.find()){
						doublePairs.setDouble2(func.f(Double.POSITIVE_INFINITY));
						context.write(word.set(matcher.group().toLowerCase()), doublePairs);
					}
				}
        ...
        @Override
            public void reduce(Text key, Iterable<Text> values,
				 ...
				 for (DoublePair value: values){
					aword += pair.getDouble1();
					sword += pair.getDouble2();
				 }
				context.write(key, new DoublePairs(aword, sword); 
    ...
    public static class Reduce1 extends Reducer<Text, DoublePair, DoubleWritable, Text> {
        @Override
            public void reduce(Text key, Iterable<Text> values,
				...
				for (DoublePair value: values){
					aword += value.getDouble1();
					sword += value.getDouble2();
				}
				if (sword > 0.0){
					result = sword/aword * Math.pow(Math.log(sword), 3);
				}
				doubleWritable.set(result);
				context.(doubleWritable, key);
            }
    }
